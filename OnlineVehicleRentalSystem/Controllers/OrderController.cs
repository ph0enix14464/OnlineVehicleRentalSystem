    ...
    public class OrderController : Controller
    {
        ...
        public ActionResult ViewOrder(int id)
        {
            using (OnlineVehicleDBEntities db = new OnlineVehicleDBEntities())
            {
                ...
                {
                    lstod.Add(new OrderDetailsViewModel() { ItemId = Convert.ToInt32(item.VehicleId), ItemName = item.tblVehicle.Title, Quantity = Convert.ToInt32(item.Quantity), UnitPrice = Convert.ToDecimal(item.UnitPrice) });
                }
